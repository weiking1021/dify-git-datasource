...
3. (Optional, required only for the "verify against existing documents" feature below) Set **Dify API base URL** and **Dify API key**. The base URL defaults to `http://api:5001`, a best-effort guess for the standard self-hosted docker-compose layout - Dify's plugin sandbox deliberately never exposes its own instance's address to plugin code, so this can't be auto-detected and may need changing for your deployment (Kubernetes, serverless plugin execution, a renamed `api` service, etc.).
4. In a Knowledge Pipeline, add this datasource and configure, per node:
   ...
   - **Verify against existing knowledge base documents** - off by default. Every run simply lists whatever matching files git currently has; nothing is tracked, so nothing can get out of sync. Turn this on to instead query the target Knowledge Base's actual documents (via Dify's own Dataset API) and skip files that already exist there - this requires the Dify API key/base URL above and the dataset ID below.
   - **Target Knowledge Base dataset ID** - required only when the toggle above is on. Copy it from the Knowledge Base's own settings/API page.
   - **Append content hash to file names** - an independent setting. When on, each file's listed name gets a short hash of the file's own content before its extension (`guide@a1b2c3d4e5f6.md` instead of `guide.md`). This is a git blob hash (content-based), not a commit hash - it changes only when this file's content changes, never when other files in the repo change. Combined with "verify against existing documents", this makes an updated file re-sync as a new document instead of being skipped forever once any version of it exists - at the cost of leaving the old version's document behind in the Knowledge Base.
...
- "Verify against existing knowledge base documents" matches by document name, so if left combined with "append content hash to file names" off, a file is only ever ingested once - later edits to it are not re-synced. Turning the content-hash tag on fixes that, but leaves old versions' documents in the Knowledge Base rather than replacing them (unless "archive superseded versions" is also on).
